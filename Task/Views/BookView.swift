...
struct BookView: View {
        ...
        VStack{
            Form {
                ...
                Section(header: Text("Date of packaging:(dd.mm.yyyy)"), content: {
                    TextField(book.DateOfPackaging, text: $textFieldtextDateOfPackaging)
                        .padding(.horizontal)
                        .frame(height: 45)
                        .background(Color(#colorLiteral(red: 0.1137, green: 0.0902, blue: 0.1294, alpha: 0.1)))
                        .cornerRadius(10)
                ...
                .alert(isPresented: $showingAlert) {
                    Alert(title: Text("Invalid input"), message: Text("Values Amount and Number of pages must be numerical and positive"), dismissButton: .default(Text("Got it!")))
                }
            }
        }
    }
    func saveButtonPressed(){
        do{
            try checkData()
            book.setBook(price: textFieldtextPrice, country: textFieldtextCountryOfOrigin, dateP: textFieldtextDateOfPackaging, description: textFieldtextDescription, number: textFieldtextNumber, house: textFieldtextPublishingHouse, authors: textFieldtextAuthors)
